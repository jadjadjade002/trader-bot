...
//|          v16.00 Velocity High-Frequency M1 Scalper Framework     |
...
//+------------------------------------------------------------------+
#property copyright "QuantumTitan Institutional Quant Framework v16.00 Velocity"
#property link      "https://github.com/jadjadjade002/trader-bot"
#property version   "15.00"
#property description "v16.00 Velocity M1 High-Frequency Scalper: LazyBear Squeeze Momentum, EMA14/50 Retest, Wick-Fill, +180pt Quick TP"
...
input ulong    InpMagicNumber          = 991601;     // Magic Number (Isolated M1 Velocity ID)
...
void RenderHUD(const SqueezeState &sqz, double fastEma, double slowEma, int activeTrades, double pnl)
{
   ...
   lines[0] = ">> QUANTUM TITAN v16.00 VELOCITY [M1 RAPID SCALPER] <<";
...
int OnInit()
{
   ...
   PrintFormat("⚡ QuantumTitan v16.00 Velocity initialized on %s M1 (Magic: %d)", _Symbol, InpMagicNumber);
...
void OnTick()
{
   ...
   if(signalBuy)
   {
      ...
      if(g_trade.Buy(strictLot, _Symbol, ask, sl, tp, "QT16_Velocity_Buy"))
      {
   ...
   else if(signalSell)
   {
      ...
      if(g_trade.Sell(strictLot, _Symbol, bid, sl, tp, "QT16_Velocity_Sell"))
      {
         g_lastBarTime = currentBarTime;
